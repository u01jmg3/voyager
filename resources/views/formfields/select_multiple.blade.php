...
@if(isset($options->relationship) && !method_exists( $dataType->model_name, \Str::camel($row->field) ) )
...
<select class="form-control select2" name="{{ $row->field }}[]" multiple>
    @if(isset($options->relationship))
        ...
        @if( method_exists( $dataType->model_name, \Str::camel($row->field) ) )
            <?php $selected_values = isset($dataTypeContent) ? $dataTypeContent->{\Str::camel($row->field)}()->pluck($options->relationship->key)->all() : []; ?>
            <?php
            ...
            if (isset($dataTypeContent) && method_exists($dataTypeContent, $relationshipListMethod)) {
                ...
                $relationshipClass = get_class(app($dataType->model_name)->{\Str::camel($row->field)}()->getRelated());
